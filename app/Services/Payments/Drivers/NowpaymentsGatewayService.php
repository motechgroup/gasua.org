...
class NowpaymentsGatewayService implements PaymentGatewayInterface
{
    ...
    public function initiatePayment(Donation $donation, array $extraInput = []): array
    {
        ...
        // If credentials demo or real request
        ...
        try {
            $payload = [
                'price_amount' => $donation->amount,
                'price_currency' => strtolower($donation->currency),
            ...
            $res = Http::withHeaders(['x-api-key' => $apiKey])->post('https://api.nowpayments.io/v1/payment', $payload);
            $json = $res->json();
            ...
            if ($res->successful() && isset($json['payment_id'])) {
                return [
                    ...
                    'redirect_url' => null,
                    'crypto_address' => $json['pay_address'] ?? null,
                    'crypto_currency' => strtoupper($json['pay_currency'] ?? 'Crypto'),
        ...
        return [
            ...
            'instructions' => 'Failed to initialize NOWPayments transaction.',
            'raw_response' => [],
        ];
    }
